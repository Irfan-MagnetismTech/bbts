...
(function ($, undefined) {
    ...
    function insertStep(wizard, options, state, index, step) {
        ...
        if (
            state.currentIndex !== state.stepCount &&
            state.currentIndex >= index
        ) {
            state.currentIndex++;
            saveCurrentStateToCookie(wizard, options, state);
    ...
    function loadAsyncContent(wizard, options, state) {
        if (state.stepCount > 0) {
            ...
            if (!options.enableContentCache || !currentStep.contentLoaded) {
                switch (
                    getValidEnumValue(contentMode, currentStep.contentMode)
                ) {
                    ...
                    case contentMode.async:
                        ...
                        $.ajax({
                            ...
                            currentStepContent
                                ...
                                ._aria("busy", "false")
                                .data("loaded", "1");
                            wizard.triggerHandler("contentLoaded", [
                                currentIndex,
                            ]);
                        });
                        break;
    ...
    function refreshPagination(wizard, options, state) {
        if (options.enablePagination) {
            var finish = wizard.find(".actions a[href$='#finish']").parent(),
                ...
                finish.find("a").attr("id", "finishButton")
    ...
    function renderTitle(wizard, options, state, header, index) {
        ...
        stepItem._enableAria(
            options.enableAllSteps || state.currentIndex > index
        );
    ...
    function validateArgument(argumentName, argumentValue) {
        if (argumentValue == null) {
            throwError(
                "The argument '{0}' is null or undefined.",
                argumentName
            );
        }
    }
