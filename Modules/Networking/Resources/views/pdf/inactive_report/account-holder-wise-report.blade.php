            ...
            <tbody>
                @foreach ($permanently_inactive_clients as $key => $inactive_clients)
                    ...
                    <tr style="background-color: #f3f3f3;">
                        ...
                        <td style="font-weight: bold">Connectivity Point</td>
                        <td style="font-weight: bold">Branch</td>
                        <td style="font-weight: bold">Thana</td>
                        <td style="font-weight: bold">Product</td>
                        <td style="font-weight: bold">Quantity</td>
                        <td style="font-weight: bold">Equipment</td>
                        <td style="font-weight: bold">OTC</td>
                        <td style="font-weight: bold">Monthly Revenue</td>
                    ...
                    @foreach ($inactive_clients as $client)
                        ...
                        @for ($i = 0; $i < $max_row; $i++)
                            <tr>
                                @if ($i == 0)
                                    <td rowspan="{{ $max_row }}" style="font-size: 12px;">
                                        {{ $client['client_name'] }}</td>
                                    <td rowspan="{{ $max_row }}" style="font-size: 12px;">
                                        {{ $client['connectivity_point'] }}</td>
                                    <td rowspan="{{ $max_row }}" style="font-size: 12px;">{{ $client['branch'] }}
                                    </td>
                                    <td rowspan="{{ $max_row }}" style="font-size: 12px;">{{ $client['thana'] }}
                                    </td>
                                @endif
                                @if (isset($client['sale_product_details'][$i]))
                                    <td style="font-size: 12px;">
                                        {{ $client['sale_product_details'][$i]->product->name }}</td>
                                    <td style="font-size: 12px;">{{ $client['sale_product_details'][$i]->quantity }}
                                    </td>
                                @else
                                    <td>-</td>
                                    <td>-</td>
                                ...
                                @endif
                                @if ($i == 0)
                                    <td rowspan="{{ $max_row }}" style="font-size: 12px;">{{ $client['otc'] }}
                                    </td>
                                    <td rowspan="{{ $max_row }}" style="font-size: 12px;">{{ $client['mrc'] }}
                                    </td>
                                    <td rowspan="{{ $max_row }}" style="font-size: 12px;">{{ $client['reason'] }}
                                    </td>
                                @endif
                            </tr>
                        @endfor
