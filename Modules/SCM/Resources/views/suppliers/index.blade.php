        ...
        <table id="dataTable" class="table table-striped table-bordered">
            ...
            @foreach($suppliers as $key => $supplier)
                <tr>
                    ...
                    <td class="text-center">{{$supplier->unit->name ??''}}</td>
                    <td class="text-center">{{$supplier->type ??''}}</td>
                    <td class="text-center">{{$supplier->code}}</td>
                    <td>
                        <div class="icon-btn">
                            <nobr>
                                <a href="{{ route('suppliers.edit', $supplier->id) }}" data-toggle="tooltip" title="Edit" class="btn btn-outline-warning"><i class="fas fa-pen"></i></a>
                                <form action="{{ url("admin/suppliers/$supplier->id") }}" method="POST" data-toggle="tooltip" title="Delete" class="d-inline">
                                    @csrf
                                    @method('DELETE')
