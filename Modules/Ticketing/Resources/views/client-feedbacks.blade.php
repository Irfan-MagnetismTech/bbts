    ...
    <div class="dt-responsive table-responsive">
        <table id="dataTable" class="table table-striped table-bordered">
            ...
            <tbody>
                ...
                <tr>
                    <td>{{ $loop->index + 1 }}</td>
                    <td><a href="{{ route('support-tickets.show', ['support_ticket' => $feedback->first()->supportTicket->id]) }}">{{ $feedback->first()->supportTicket->ticket_no }}</a></td>
                    <td>{{ $feedback->first()->clientDetail->client->name }}</td>
                    <td>{{ $feedback->first()->clientDetail->link_name }} / {{ $feedback->first()->clientDetail->link_id }}</td>
                    <td class="text-left">
                        @foreach ($feedback as $item)
                            <div class="pb-3">
                                <strong>Rating:</strong> {{ $item->rating }} <br>
                                <strong>Comment:</strong> {{ $item->feedback }} <br>
                                <small>Date: {{ \Carbon\Carbon::parse($item->created_at)->format('d/m/Y \a\t H:i a') }}</small>
                            </div>
                        @endforeach
                    </td>
                </tr>
