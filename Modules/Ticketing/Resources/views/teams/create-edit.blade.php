...
@section('breadcrumb-button')
    <a href="{{ route('support-teams.index') }}" class="btn btn-out-dashed btn-sm btn-warning"><i
                    ...
                    <div class="row">
                            ...
                            <select name="department_id" id="department_id" class="form-control">
                                ...
                                @foreach ($departments as $department)
                                    <option value="{{ $department->id }}"
                                        {{ old('department_id', !empty($supportTeam) ? $supportTeam?->department_id : '') == $department->id ? 'selected' : '' }}>
                        ...
                        <div class="form-group col-6">
                            ...
                            <input type="text" class="form-control" id="user_name" name="user_name"
                                ...
                                value="{{ old('user_name') ?? (!empty($supportTeam) ? $supportTeam?->user?->name : '') }}"
                                placeholder="Department Head" required>
                            ...
                            <tbody>
                                @if (empty($supportTeam) || empty($supportTeam?->supportTeamMembers))
                                    <tr>
                                        <td>1</td>
                                        <td>
                                            <input type="text" value="" placeholder="Search Employee"
                                                class="user-search form-control" />
                                            <input type="hidden" name="users_id[]" value=""
                                                placeholder="Search Employee" class="member_id form-control" />
                                        </td>
                                        <td></td>
                                        <td>
                                    ...
                                    @foreach ($supportTeam->supportTeamMembers as $teamMember)
                                        <tr>
                                            ...
                                            <td>
                                                ...
                                                <input type="text" value="{{ $teamMember?->user?->name }}"
                                                    placeholder="Search Employee" class="user-search form-control" />
                                                <input type="hidden" name="users_id[]" value="{{ $teamMember->user_id }}"
                                                    placeholder="Search Employee" class="member_id form-control" />
                                            </td>
                                            ...
                                            <td>
                                                <select name="type[]" class="form-control">
                                                    @foreach ($levels as $id => $level)
                                                        <option value="{{ $id }}"
                                                            {{ $teamMember?->type == $id ? 'selected' : null }}>
                                                            {{ $level }}</option>
                                                    @endforeach
                                                </select>
                                            </td>
