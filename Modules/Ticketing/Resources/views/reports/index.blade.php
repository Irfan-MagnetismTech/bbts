...
@section('breadcrumb-button')
    <a href="{{ route('report-index') }}" class="btn btn-out-dashed btn-sm btn-warning"><i class="fas fa-database"></i></a>
@endsection
@section('sub-title')
...
@section('content')
        ...
        <div class="row">
                ...
                <div class="form-group">
                    ...
                    <select name="status" id="status" class="form-control">
                        ...
                        @foreach (config('businessinfo.ticketStatuses') as $ticketStatus)
                            <option value="{{ $ticketStatus }}" {{ $request->status == $ticketStatus ? 'selected' : '' }}>
                                {{ $ticketStatus }}</option>
                        @endforeach
                    </select>
                </div>
                    ...
                    <select name="ticket_source_id" id="ticket_source_id" class="form-control">
                        ...
                        @foreach ($ticketSources as $source)
                            <option value="{{ $source->id }}"
                                {{ $request->ticket_source_id == $source->id ? 'selected' : '' }}>{{ $source->name }}
                            </option>
                        @endforeach
                    </select>
                </div>
                    ...
                    <select name="support_complain_type_id" id="support_complain_type_id" class="form-control">
                        ...
                        @foreach ($complainTypes as $type)
                            <option value="{{ $type->id }}"
                                {{ $request->support_complain_type_id == $type->id ? 'selected' : '' }}>
                                {{ $type->name }}</option>
                        @endforeach
                    </select>
                </div>
            ...
            <div class="col-md-3">
                <div class="form-group">
                    ...
                    <select name="search_by_days" id="search_by_days" class="form-control">
                        ...
                        <option value="1" {{ request()->search_by_days == 1 ? 'selected' : '' }}>1 Days</option>
                        <option value="2" {{ request()->search_by_days == 2 ? 'selected' : '' }}>2 Days</option>
                        <option value="3" {{ request()->search_by_days == 3 ? 'selected' : '' }}>3 Days</option>
                        <option value="4" {{ request()->search_by_days == 4 ? 'selected' : '' }}>4 Days</option>
                        <option value="5" {{ request()->search_by_days == 5 ? 'selected' : '' }}>5 Days</option>
                        <option value="6" {{ request()->search_by_days == 6 ? 'selected' : '' }}>6 Days</option>
            ...
            <div class="col-md-3">
                <div class="form-group my-4 row">
                    ...
                    <div class="col-md-6">
                        <input id="filterData" type="button" value="Search"
                            class="btn btn-outline-primary btn-sm col-12">
                    </div>
                </div>
            </div>
            <div class="col-md-3">
                <div class="form-group my-4 row">
                    <div class="col-md-6">
                        <button type="button" id="pdfDownload" onclick="reportDownload('pdf')"
    ...
    <div class="dt-responsive table-responsive">
            ...
            <tbody>
                @foreach ($supportTickets as $supportTicket)
                    <tr>
                        ...
                        <td>{{ $supportTicket->duration ?? '' }}</td>
                        <td>{{ $supportTicket->client->client_name ?? '' }}</td>
        ...
        <div class="row mt-2 mb-4">
            <div class="col-md-4">
                <div class="form-group my-4 row">
                    <form action="{{ route('filtered-report-download') }}" method="post"
                        id="datatableFilteredReportDownload" class="col-12 row">
                        <div class="col-md-6">
                            @csrf
                            <button type="button" onclick="dttablereport('pdf')"
                                class="btn btn-outline-danger btn-sm col-12">
                                PDF Download
                                <i class="far fa-file-pdf"></i>
                            </button>
