<script>
    ...
    $('.offer_otc').on('input', function() {
        // calculateOtc($(this));
        var offer_otc = $(this).val() || 0;
        var client_equipment_amount = $(this).closest('tr').find('.client_equipment_amount').val() || 0;
    ...
    function calculateTotalOtc(event) {
        ...
        event.closest('.mainRow').find('.offer_otc').each(function() {
            let offer_otc = $(this).val();
            total_otc += parseFloat(offer_otc) || 0;
        });
        event.closest('.mainRow').find('.total_cost_otc').each(function() {
            let cost_otc = $(this).val() || 0;
    ...
    function calculateEquipmentPrice(event) {
        ...
        event.closest('tr').find('.equipment_total_otc').val(total_amount.toFixed(2));
    }
    ...
    function calculateEquipmentROI(value) {
        let equipment_otc = value.closest('tr').find('.equipment_offer_price').val() || 0;
        let month = value.closest('tr').find('.month').val() || 0;
        let equipment_invest = value.closest('tr').find('.equipment_invest').val() || 0;
