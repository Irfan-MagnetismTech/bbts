...
@section('content')
        @if($formType == 'edit')
            {!! Form::open(array('url' => "admin/units/$unit->id",'method' => 'PUT')) !!}
            <input type="hidden" name="id" value="{{old('id') ? old('id') : (!empty($unit->id) ? $unit->id : null)}}">

        @else
            {!! Form::open(array('url' => "admin/units",'method' => 'POST')) !!}
        @endif
        <div class="row">
                ...
                @foreach($units as $key => $data)
                    <tr>
                        ...
                        <td>
                            <div class="icon-btn">
                                <nobr>
                                    <a href="{{ url("admin/units/$data->id/edit") }}" data-toggle="tooltip" title="Edit" class="btn btn-outline-warning"><i class="fas fa-pen"></i></a>
                                    {!! Form::open(array('url' => "admin/units/$data->id",'method' => 'delete', 'class'=>'d-inline','data-toggle'=>'tooltip','title'=>'Delete')) !!}
